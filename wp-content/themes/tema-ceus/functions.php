...
function update_custom_field_on_save($post_id) {
    // Verifica se é uma revisão para evitar loop infinito
    if (wp_is_post_revision($post_id)) {
        return;
    }

    // Verifica se o campo customizado 'data_tipo_de_data' existe
    $data_tipo_de_data = get_post_meta($post_id, 'data_tipo_de_data', true);
    
    // Se o campo 'data_tipo_de_data' for igual a 'data'
    if ($data_tipo_de_data === 'data') {
        // Pega o valor do campo 'data_data'
        $data_data = get_post_meta($post_id, 'data_data', true);

        // Insere o valor no campo 'data_data_final'
        update_post_meta($post_id, 'data_data_final', $data_data);
    }
}
...
add_action('wp_trash_post', 'fix_title_before_trash', 1);
