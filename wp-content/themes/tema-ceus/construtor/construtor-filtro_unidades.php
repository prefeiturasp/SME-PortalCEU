                            ...
                            <?php if ( $dres ) : ?>
                                <div class="col-12 col-md-4 mb-sm-3">                    
                                    <select class="form-control input-top-search" onchange="location = this.value;">
                                        <option value="<?= get_the_permalink(); ?>?formato-exibicao=mapa">Buscar por Zona de Setorização</option>
                                        <option value="<?= get_the_permalink(); ?>?formato-exibicao=mapa&zona=todas" <?= $_GET['zona'] == 'todas' ? 'selected' : ''?>>Todas</option>
