pipeline {
    ...
    stages {
        ...
        stage('Build') {
          ...
          steps {
            script {
              imagename1 = "registry.sme.prefeitura.sp.gov.br/wordpress/${env.branchname}/ceu"
            ...
            steps {
                script{
                    ...
                    withCredentials([file(credentialsId: 'config_wordpress', variable: 'config')]){
                        ...
                        sh 'kubectl rollout restart deployment/prod-ceu -n prod-ceu'
                        sh('if [ -f '+"$home"+'/.kube/config ];then rm -f '+"$home"+'/.kube/config; fi')
                   }
                }
            }           
            ...
            steps {
                script{                 
                    withCredentials([file(credentialsId: 'config_wordpress', variable: 'config')]){
                        sh('if [ -f '+"$home"+'/.kube/config ];then rm -f '+"$home"+'/.kube/config; fi')
                        sh('cp $config '+"$home"+'/.kube/config')
                        sh 'kubectl rollout restart deployment/hom-ceu -n hom-ceu'
                        sh('if [ -f '+"$home"+'/.kube/config ];then rm -f '+"$home"+'/.kube/config; fi')
                   }
                }
            }           
